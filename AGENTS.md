...
This checkout contains no Git history from which to infer an established convention. Use short, imperative, scoped subjects, for example `Core: handle missing sensor values`. Pull requests should explain the behavior change, list validation commands and hardware/admin conditions, link relevant issues, and include screenshots for UI changes. Call out new localization keys and configuration-schema changes. Never commit local `config.json`, `crash.log`, build artifacts, secrets, or unsigned replacement drivers.

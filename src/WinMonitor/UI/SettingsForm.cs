...
public sealed partial class SettingsForm : Form
{
    ...
    private AppConfig Config => _draft.Draft;

    public SettingsForm(WinMonitorContext ctx)
    {
        _ctx = ctx;
        _draft = new SettingsDraft(ctx.Config);
    ...
    {
        if (_draft.Rebase(_ctx.Config) && refreshControls) LoadAllTabs();
    }

    private AppConfig BuildMergedConfig() => _draft.BuildMergedConfig(_ctx.Config);

    private void ResetDraftFromLiveConfig() => _draft.Reset(_ctx.Config);

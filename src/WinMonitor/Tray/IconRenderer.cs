...
/// (SM_CXSMICON/SM_CYSMICON — 24x24 at 150% scaling), so the taskbar shows our pixels 1:1;
/// rendering 32x32 and letting the shell resample down was the main source of blur.
...
public static class IconRenderer
{
    ...
    // ~55% alpha black rim drawn 8-way behind the glyphs so text stays readable on both
    // light and dark taskbars.
    private static readonly Color HaloColor = Color.FromArgb(140, 0, 0, 0);
        ...
        lock (_gate)
        {
            ...
            try
            {
                using (var g = Graphics.FromImage(bmp))
                {
                    ...
                    // AntiAliasGridFit, NOT ClearType: ClearType's subpixel coverage needs an
                    // opaque background — against transparent alpha it produces color-fringed,
                    // washed-out glyphs.
                    g.TextRenderingHint = TextRenderingHint.AntiAliasGridFit;
                    Font font = PickFont(g, text, bold, spark);

                    // With a sparkline the number is confined to the upper band; otherwise it
                    // uses the whole canvas (unchanged behaviour).
                    float textHeight = spark ? CanvasSize * (1f - SparklineBandFraction) : CanvasSize;
                    var bounds = new RectangleF(0f, 0f, CanvasSize, textHeight);

                    // Contrast halo: the same string 8-way offset by 1px behind the fill.
                    // (Invisible over a dark badge, so it is safe to draw unconditionally.)
                    SolidBrush haloBrush = GetBrush(HaloColor);
                    for (int dy = -1; dy <= 1; dy++)
                    {
                        for (int dx = -1; dx <= 1; dx++)
                        {
                            if (dx == 0 && dy == 0) continue;
                            var haloBounds = new RectangleF(dx, dy, CanvasSize, textHeight);
                            g.DrawString(text, font, haloBrush, haloBounds, _centerFormat);
                        }
                    }

                    g.DrawString(text, font, GetBrush(fg), bounds, _centerFormat);

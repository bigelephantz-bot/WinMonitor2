...
- `TrayIconManager` publishes an immutable set of sparkline sensor IDs during rebuild. Valid samples for those IDs schedule a coalesced UI update even when the numeric value is unchanged; the full-window hash still prevents replacing an unchanged HICON. Numeric-only icons and unchanged unrelated/sparse samples retain their idle fast path.
...
## Program.cs wiring
- `tools/ResourceProbe` hosts the real context/UI with isolated config and the app's GC/PGO settings for resource measurements. Its internal constructor disables only startup registration, global hotkeys and the activation pipe; the public constructor retains all normal behavior. The probe closes windows between phases and shuts down the sensor service normally. See `docs/RESOURCE-USAGE-20260924.md`.

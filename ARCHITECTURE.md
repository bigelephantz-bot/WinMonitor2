...
- **Layout identity covers meaning, not just naming.** The sidecar fingerprint includes `Id`, `HardwareName`, `Name`, `DisplayName` and **`Quantity`** — an EC sensor id is `/ec/reg/XX/{Kind}` and carries no quantity, so switching one from Fan to Temperature would otherwise keep writing °C into an RPM column under an unchanged header.

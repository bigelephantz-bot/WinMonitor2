...
- **SettingsForm is split into partial classes** (`SettingsForm.cs` shell + `.General/.TrayIcons/.Sensors/.Alerts/.Profiles/.Diagnostics.cs`) — put tab edits in that tab's partial. Cancel semantics: a JSON snapshot is taken at open and restored on Cancel/X; every edit mutates `ctx.Config` live and only `ctx.ApplySettings()` propagates. The Diagnostics tab deliberately reads the **live service**, not the settings draft.
